...
3. Do not add comments unless they communicate information that cannot be obviously inferred from the code.

<!-- Tech Stack and Repos-->

1. Always use pnpm or bun (if its a bun project) or yarn if there is a yarn.lock file present. Never npm
2. Unless specifically stated, neglect the schooldesk_old_backend folder

<!-- testing -->

For backend tasks, before adding each new functionality, determine whether the nature of the tasks makes it easy enough to practice TDD.
If it does, write full e2e tests, including edge cases, before implementing the functionality.
If it doesn't go ahead to implement the functionality first, and then proceed wo write full e2e tests. Don't write too many unnecessary tests that add no value. Focus on a few tests that check core functionality and then edge cases.

However we want to avoid a bloat of tests. We do not want redundant tests. If a specific case is tested, there is no need to write more tests to retest it, unless they are covering a DIFFERENT edge case.
...
- check for typescript errors using "npx tsc -b"
